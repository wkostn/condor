...
class PydanticAIClient:
    ...
    async def prompt_stream(self, text: str) -> AsyncIterator[ACPEvent]:
        ...
        try:
            ...
            # Map original tool_call_id → emitted tool_id so we can
            # attach the result text when the next ModelRequestNode arrives.
            ...
            async with self._agent.iter(text) as run:
                async for node in run:
                    ...
                    if isinstance(node, ModelRequestNode):
                        ...
                        # After a CallToolsNode, PydanticAI executes the
                        # tools and packs ToolReturnPart entries into the
                        # next ModelRequest.  Emit them as ToolCallUpdates.
                        if _pending and hasattr(node, "request") and node.request:
                            for part in node.request.parts:
                                if not isinstance(part, ToolReturnPart):
                                    continue
                                tc_id = part.tool_call_id
                                emitted_id = _pending.pop(tc_id, None)
                                if emitted_id is None:
                                    continue
                                output_text = str(part.content) if part.content else ""
                                if len(output_text) > 2000:
                                    output_text = output_text[:2000] + "…(truncated)"
                                yield ToolCallUpdate(
                                    tool_call_id=emitted_id,
                                    status="completed",
                                    output=output_text,
                                )
                            # Any remaining unmatched calls — mark completed
                            # without output so snapshots never show "in_progress"
                            for leftover_id in _pending.values():
                                yield ToolCallUpdate(
                                    tool_call_id=leftover_id,
                                    status="completed",
                                )
                            _pending.clear()
                        ...
                        for part in node.model_response.parts:
                            if isinstance(part, TextPart) and part.content:
                                ...
                                original_tc_id = part.tool_call_id
                                tool_id = original_tc_id or uuid.uuid4().hex[:12]
                                ...
                                # Remember so we can match the ToolReturnPart
                                if original_tc_id:
                                    _pending[original_tc_id] = tool_id
