...
class TickEngine:
    ...
    def __post_init__(self):
        ...
        if self.is_experiment:
            ...
            # Use persistent agent ID if configured, else fall back to session-specific ID
            # This ensures executors are shared across sessions of the same agent
            if self.strategy.id:
                self.agent_id = self.strategy.id
            else:
